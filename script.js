...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // --- Pull-to-Refresh Logic (ဒီနေရာမှာ စထည့်ပါ) ---
    ...
    if (isMobileDevice) {
        ...
        // Desktop အတွက် Scroll စစ်ဆေးချက်
...
// 1. Global Scope မှာ Promise ကို အရင်ပတ်ထားမယ်
    ...
    const checkInterval = setInterval(() => {
        if (typeof supabase !== 'undefined' && window.supabaseUrl && window.supabaseKey) {
            ...
            try {
                ...
                // 2. Client အဆင်သင့်ဖြစ်ပြီဆိုတာကို Resolver ကနေ အကြောင်းကြားမယ်
...
async function uploadAndPost() {
    ...
    const btnPost = document.querySelector('button[onclick="uploadAndPost()"]'); // Post တင်မည့်ခလုတ်
    
    // Input values
    const files = Array.from(mediaInput.files);
    const text = postContent.value.trim();

    // ၁။ Validation: စာလည်းမရှိ၊ ဖိုင်လည်းမရှိလျှင် မတင်ပါ
    if (!text && files.length === 0) {
        return showToastMessage("ပို့စ်တင်ရန် တစ်ခုခုရေးပါ သို့မဟုတ် ဖိုင်ရွေးပါ", "warning");
    }

    // ၂။ Session & Profile စစ်ဆေးခြင်း
    const { data: { session } } = await window.supabase.auth.getSession();
    ...
    try {
        ...
        const isBadge = profile?.has_badge || false;
        const maxFiles = isBadge ? 10 : 1;

        if (files.length > maxFiles) {
            throw new Error(`သင်သည် ${maxFiles} ဖိုင်သာ တင်ခွင့်ရှိပါသည်။`);
        }

        let finalMediaUrls = [];
        let finalMediaType = "text";

        // ၃။ ဖိုင်ပါလျှင် API သို့အရင်ပို့ပြီး Bunny URLs များရယူခြင်း
        if (files.length > 0) {
            ...
            // API မှပြန်လာသော URLs များကို သိမ်းဆည်းခြင်း
            finalMediaUrls = result.urls; // result.urls သည် array ဖြစ်ရမည်
            
            // Media Type သတ်မှတ်ခြင်း (ပထမဆုံးဖိုင်ကို ကြည့်၍ ဆုံးဖြတ်ခြင်း)
        ...
        // ၄။ 🔥 Supabase Database ထဲသို့ ပို့စ်အချက်အလက်များ သိမ်းဆည်းခြင်း
        const postData = {
            user_id: user.id,
        ...
        // ၅။ အောင်မြင်မှု Feedback ပေးခြင်းနှင့် UI Reset လုပ်ခြင်း
        showToastMessage("ပို့စ်တင်ပြီးပါပြီ Senior!", "success");
        
        // Reset Inputs
        postContent.value = "";
        mediaInput.value = "";
        if (typeof clearPreview === 'function') clearPreview(); // Preview ရှင်းထုတ်ခြင်း
        
        // Feed ကို Refresh လုပ်ခြင်း
        if (typeof refreshFeed === 'function') {
            await refreshFeed();
        } else if (typeof loadPosts === 'function') {
            await loadPosts(targetTable, false);
        ...
        // ခလုတ်ကို မူလအတိုင်းပြန်ထားခြင်း
        if (btnPost) {
