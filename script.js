...
async function loginWithGoogle() {
    try {
        ...
        showToastMessage("OTP ပို့ပြီးပါပြီ", "success");}
}
...
async function sendOTP() {
    ...
    try {
        ...
        showToastMessagealert("Error: " + error.message);
    }
}
...
async function handleDeviceAutoLogin() {
    try {
        ...
        // ၃။ Anonymous Auth နဲ့ Login ဝင်မယ်
        // ဒီ function က အကောင့်မရှိရင် အသစ်ဖွင့်ပေးပြီး ရှိရင် session ပြန်ပေးပါတယ်
        ...
        // ၄။ Profiles Table မှာ device_id နဲ့ သွားချိတ်မယ် (မရှိရင် Create လုပ်မယ်)
        await syncAnonymousProfile(user.id, deviceId);

    } catch (err) {
        console.error("❌ Auto Login Error:", err.message);
        showToastMessage("အလိုအလျောက် Login ဝင်လို့မရပါဘူး Senior");
    }
}
...
async function uploadAndPost() {
    ...
    // selectedFiles (ပထမ code မှ variable) သို့မဟုတ် input files ကို ယူခြင်း
    ...
    try {
        // ၂။ User Rank စစ်ဆေးခြင်း (Validation)
        ...
        if (!text && files.length === 0) {
            throw new Error("စာ သို့မဟုတ် ဖိုင်တစ်ခုခု ထည့်ပေးပါ Senior");
        }
        ...
        // ၃။ Media Upload Logic (Parallel Processing)
        const uploadPromises = files.map(async (file) => {
            ...
            if (isVideo) {
                if (file.size > maxVideoSize) {
                    throw new Error(`ဗီဒီယို ${file.name} က ${isPremium ? '60MB' : '20MB'} ထက် ကျော်နေပါတယ်`);
                }
        ...
        // Form အကုန်လုံးကို Clean လုပ်မယ်
        postContent.value = "";
        mediaInput.value = "";
        if (typeof selectedFiles !== 'undefined') selectedFiles = []; // Array ကိုပါ reset လုပ်ခြင်း
...
function removeSingleFile(index) {
    ...
    // ၂။ HTML Input (File Input) ကိုပါ တစ်ခါတည်း ရှင်းမယ်
    // ဒါမှ နောက်တစ်ခါ ပုံပြန်ရွေးရင် error မတက်မှာပါ
...
document.addEventListener('DOMContentLoaded', async () => {
    // အရင်ဆုံး Auto Login လုပ်မယ်
    await handleDeviceAutoLogin();
    
    // ပြီးမှ ကျန်တဲ့ Observer တွေနဲ့ Post တွေကို Load လုပ်မယ်
