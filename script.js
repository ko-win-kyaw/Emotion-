...
function refreshFeed() {
    ...
    if (container) {
        // ၃။ ပို့စ်ဟောင်းတွေကို ရှင်းထုတ်ပြီး Loading Spinner (သို့မဟုတ်) စာသား ပြမယ်
        container.innerHTML = `
    ...
    if (posts.length >= 10) {
        ...
        <div id="scroll-trigger" style="text-align:center; margin:30px 0; padding-bottom: 50px;">
            <button id="btnLoadMore" 
                onclick="loadMorePosts('${targetTable}')" 
...
const targetTable = window.MAIN_POST_TABLE || 'posts';
    ...
    await loadPosts(false, targetTable);
}
...
window.showAllComments = async function(postId) {
    ...
    try {
        ...
        if (comments && comments.length > 5) {
            ...
            // အကယ်၍ ထပ်ပြစရာ comment မရှိတော့ရင် (သို့မဟုတ်) data error တစ်ခုခုကြောင့် comments မရှိရင်
...
async function uploadAndPost() {
    ...
    try {
        ...
        // ၂။ Media Upload Logic (Parallel Optimization)
        // Promise.all သုံးပြီး ပုံ/ဗီဒီယို အားလုံးကို အပြိုင် Upload တင်ပါမယ်
...
supabase.auth.onAuthStateChange(async (event, session) => {
    ...
    try {
        ...
        if (isBanned) {
            ...
            return showToastMessage("သင့်အကောင့် ပိတ်ပင်ခံထားရပါသည် ");
        }
        ...
        if (user.email !== ADMIN_EMAIL && window.currentUserData) {
            if (currentDevId !== "timeout_id" && 
                ...
                showToastMessage
                ("Account Error: Device Lock အလုပ်လုပ်နေပါသည်။ အခြားဖုန်းဖြင့် ဝင်၍မရပါ!");
                await supabase.auth.signOut();
                return;
            }
        }
...
async function syncShares() {
    ...
    // ၄။ ကျန်နေတဲ့ queue ကို localstorage မှာ update ပြန်လုပ်မယ်
    localStorage.setItem('pending_shares', JSON.stringify(shareQueue));
}

async function syncAllData() {
    // ၁။ အခြေခံ Safety Checks (Offline ဖြစ်နေရင် သို့မဟုတ် Sync လုပ်နေဆဲဆိုရင် ရပ်မယ်)
    ...
    try {
        ...
        // --- (ဂ) REACTIONS SYNC (Joker Card Logic ပေါင်းစပ်ထားသည်) ---
