...
async function loadPosts(isLoadMore = false, tableName = "posts") {
    ...
    if (!isLoadMore) {
        ...
        if (cachedData && cachedTime && (Date.now() - cachedTime < 300000)) {
            ...
            lastVisiblePost = cachedPosts[cachedPosts.length - 1]; // Pagination အတွက် နောက်ဆုံး post မှတ်ထားမယ်
            
            postsContainer.innerHTML = ''; // Loading text ဖြုတ်မယ်
            renderPostsToUI(cachedPosts, currentUid, isAdmin, tableName);
            console.log("⚡ Loaded from cache");
            return;
        }
        // Cache မရှိရင် Loading ပြမယ်
    ...
    try {
        ...
        let query = supabase
            .from(tableName)
            .select('*')
            .order('isPinned', { ascending: false }) // Pin ထားတာ အရင်ပြ
            .order('created_at', { ascending: false }) // နောက်ဆုံးတင်တာ အရင်ပြ
            .limit(10);

        // Pagination: နောက်ထပ် Load More လုပ်ရင် သုံးဖို့
        ...
        if (!isLoadMore) postsContainer.innerHTML = ''; // Loading clear လုပ်မယ်
        ...
        data.forEach(post => {
            ...
            // Intersection Observer ပြန်ချိတ်မယ် (Views count အတွက်)
        ...
        // ၆။ Save to Cache (ပထမဆုံးစာမျက်နှာကိုပဲ Cache သိမ်းတာ အကောင်းဆုံးပါ)
        if (!isLoadMore) {
            localStorage.setItem(cacheKey, JSON.stringify(data));
            localStorage.setItem(cacheTimeKey, Date.now().toString());
        }

        console.log(`✅ ${isLoadMore ? 'More' : 'Initial'} Posts Loaded`);
...
function renderPostsToUI(posts, currentUid, isAdmin, tableName) {
    ...
    html += `
        <div id="scroll-trigger" style="text-align:center; margin:20px;">
            <button onclick="loadMorePosts('${tableName}')" 
...
async function loadMorePosts(tableName = "health_posts") {
...
async function refreshPosts(tableName = "health_posts") {
    lastVisiblePost = null; // Pagination ကို အစကပြန်စဖို့
    // အရင်ရှိနေတဲ့ content တွေကို ရှင်းချင်ရင်
    const postsContainer = document.getElementById('newsFeed');
    if (postsContainer) postsContainer.innerHTML = ""; 
    
    await loadPosts(tableName);
}
...
async function deletePost(id, tableName = "health_posts") { 
    // ၁။ Confirm လုပ်ခြင်း
...
async function uploadAndPost() {
    ...
    try {
        ...
        const { error: insertError } = await supabase
            .from('page_posts') // <--- Senior ရဲ့ အဓိက Table နာမည်
            .insert([postPayload]);
        ...
        if (typeof refreshPosts === 'function') refreshPosts('page_posts');
