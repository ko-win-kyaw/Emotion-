...
async function getOrCreateUser() {
    try {
        ...
        if (savedUserId && savedName) {
            ...
            // Profile ထဲက Admin Status ကိုပါ တခါတည်း သွားပြန်စစ်ပေးခြင်း (Security အရ ပိုစိတ်ချရသည်)
...
function createPostCard(post, profile = {}) {
    ...
    if (post.media_urls && post.media_urls.length > 0) {
        ...
        if (post.media_urls.length === 1) {
            ...
            if (checkIsVideo(url)) {
                const video = document.createElement('video');
                // Bunny CDN အတွက် အတင်း Inline View ခိုင်းသည့် Query Parameter ပါထည့်သွင်းခြင်း
                video.src = url.includes('?') ? `${url}&view=true` : `${url}?view=true`;
                video.controls = true;
            ...
            for (let i = 0; i < displayCount; i++) {
                ...
                if (checkIsVideo(url)) {
                    ...
                    videoPreview.src = url.includes('?') ? `${url}&view=true` : `${url}?view=true`;
