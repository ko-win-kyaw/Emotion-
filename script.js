...
function() {
    ...
    if (event === 'SIGNED_OUT' || !user) return;

    try {
        // User Profile ကို ဆွဲထုတ်မယ်
        const { data: profile } = await supabase
            .from('profiles')
            .select('*')
            .eq('id', user.id)
        ...
        // နာမည်မပေးရသေးတဲ့ user (is_profile_setup === false) ဆိုမှသာ Modal ပြမယ်
        if (profile && profile.is_profile_setup === false) {
            if (modal) modal.style.display = 'flex';
        } else {
            // နာမည်ပေးပြီးသားဆိုရင် Modal လုံးဝ မပြတော့ဘူး
            if (modal) modal.style.display = 'none';
        }

        // User နာမည်ပြသခြင်း
        if (profile?.display_name) {
            document.getElementById('userNameDisplay').innerText = profile.display_name;
        }
...
function clearPreview() {
    ...
    if (box) {        const imgs = box.querySelectorAll('img, video');
        imgs.forEach(item => {
...
supabase.auth.onAuthStateChange(async (event, session) => {
    ...
    try {
        ...
        // --- Device Lock Logic ---
        const ADMIN_EMAIL = window.ADMIN_EMAIL || "youradmin@email.com"; // Admin email ကို သေချာထည့်ပါ
        ...
        if (!profile || profile.is_profile_setup === false) {
            if (modal) modal.style.display = 'flex';
            if (userNameDisplay) userNameDisplay.innerText = "Setting up...";
        } else {
            // Setup ပြီးသားဆိုရင် Modal လုံးဝ ထပ်မပြတော့ဘူး
            if (modal) modal.style.display = 'none';
            if (userNameDisplay) userNameDisplay.innerText = profile.display_name;
        }

        // --- Last Active Update (Upsert အစား Update သုံးတာ ပိုစိတ်ချရပါတယ်) ---
        const updatePayload = {
            last_active: new Date().toISOString()
        };
        
        if (currentDevId !== "timeout_id" && !profile?.device_id) {
            updatePayload.device_id = currentDevId; // ပထမဆုံးအကြိမ်ပဲ Device ID ကို သိမ်းမယ်
        }

        await supabase.from('profiles').update(updatePayload).eq('id', user.id);

        // Features Activate
        if (typeof loadPosts === 'function') loadPosts('health_posts', false);
        if (typeof startAutoFriendSystem === 'function') startAutoFriendSystem(user.id);
