...
function createPostCard(post, profile = {}) {
    ...
    // Admin Delete Button (If logged in user is admin)
    ...
    // Helper function: URL တစ်ခုက ဗီဒီယို ဟုတ်/မဟုတ် စစ်ဆေးရန်
    ...
    // ==================== 3. MEDIA SECTION (ဗီဒီယို ရော ပုံပါ အလုပ်လုပ်သည်) ====================
    if (post.media_urls && post.media_urls.length > 0) {
        const mediaContainer = document.createElement('div');
        mediaContainer.style.marginTop = '10px';
        
        // ကိစ္စ (က) - ဖိုင် ၁ ခုတည်း တင်ထားလျှင်
        if (post.media_urls.length === 1) {
            ...
            if (checkIsVideo(url)) {
                ...
                video.onerror = function() {
                    console.error("Video load failed for URL:", url);
                };
        ...
        // ကိစ္စ (ခ) - ဖိုင်အမြောက်အမြား (Grid System) ဖြင့် တင်ထားလျှင်
        else {
            ...
            for (let i = 0; i < displayCount; i++) {
                ...
                if (checkIsVideo(url)) {
                    // Grid ထဲမှာ ဗီဒီယိုဖြစ်နေပါက Preview Video ပြရန်
                    ...
                    // နှိပ်လိုက်လျှင် Viewer အစား ဗီဒီယိုအပြည့်ဖွင့်ကြည့်နိုင်ရန် Controls ပေးခြင်း သို့မဟုတ် လုပ်ဆောင်ချက် ထည့်နိုင်သည်
                    item.appendChild(videoPreview);
                    
                    // ဗီဒီယိုဖြစ်ကြောင်း သိသာစေရန် Play Icon Overlay လေး ထည့်ပေးခြင်း
                    ...
                    item.onclick = () => {
                        // ဗီဒီယိုကို အပြည့်ကြည့်ချင်ရင် နှိပ်လိုက်တာနဲ့ ပွင့်သွားအောင် လုပ်ပေးခြင်း
                        if (videoPreview.paused) videoPreview.play();
                        else videoPreview.pause();
                    };
                } else {
                    // ပုံဖြစ်ပါက ပုံမှန်အတိုင်း ပြရန်
                    const img = document.createElement('img');
                    img.src = url;
                    img.style.width = '100%';
                ...
                // ပုံ/ဗီဒီယို ၄ ခုထက် ကျော်သွားပါက အပေါင်းလက္ခဏာ အရေအတွက်ပြရန် (+X)
                if (i === 3 && total > 4) {
