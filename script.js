...
if (!window.supabase) {
    console.error("Supabase CDN မ load ဖြစ်ပါ");
}
...
async function handleDeviceAutoLogin() {
    try {
        ...
        // Session ရှိရင် ID ယူဖို့ မလိုအပ်တော့တဲ့အတွက် အချိန်ကုန်သက်သာစေပါတယ်
        ...
        // ၂။ Device ID (Fingerprint) ရယူမယ်
        // checkFP() သည် FingerprintJS async logic ဖြစ်ရပါမယ်
        const deviceId = await checkFP();

        // Device ID မရရင် ဒါမှမဟုတ် error ဖြစ်ရင် ရှေ့ဆက်မသွားတော့ဘဲ Error ပြမယ်
        if (!deviceId || deviceId === "error_id" || deviceId === "unknown_id") {
            throw new Error("Device ID ကို ဆွဲထုတ်လို့မရပါဘူး Senior");
        }

        console.log("📱 Device ID Verified:", deviceId);

        // ၃။ Supabase Anonymous Login ဝင်မယ်
        // Metadata ထဲမှာ device_id ကိုပါ တစ်ခါတည်း ထည့်သွင်းသိမ်းဆည်းမယ်
        ...
        // ၄။ Profile Table နဲ့ Data ပြန်ညှိမယ် (ID သိမ်းခြင်း/Profile တည်ဆောက်ခြင်း)
        if (typeof syncAnonymousProfile === "function") {
        ...
        // User ကို Error သတင်းစကားပါးမယ်
        if (typeof showToastMessage === "function") {
            showToastMessage("အလိုအလျောက် Login ဝင်လို့မရပါဘူး: " + err.message);
        }
    }
}
...
if (mUrls.length > 0) 
{
        if (mType === "video" || mUrls[0].toLowerCase().endsWith(".mp4")) {
            ...
            mediaHTML = `
                <div style="margin-top:10px; background:#000; border-radius:8px; overflow:hidden;">
                    <video class="post-video src="${safeVideo}" preload="metadata" muted playsinline webkit-playsinline 
    ...
    // --- UI RETURN ---
    return `
    <div class="post-card" id="post-${id}" data-id="${id}" style="background:white; border-radius:12px; padding:15px; margin-bottom:15px; box-shadow:0 2px 8px rgba(0,0,0,0.1); border: ${isPinned ? '1px solid #f0e6ff' : 'none'};">
