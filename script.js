...
window.BUNNY_KEY = null;
window.IMGBB_KEY = null;
window.ADMIN_EMAIL = null;
window.ADMIN_BACKUP = null;
...
async function loadConfig() {
    try {
        ...
        window.ADMIN_BACKUP = config.BACKUP_EMAIL;
window.BUNNY_STORAGE = config.BUNNY_STORAGE;
        window.BUNNY_KEY = config.BUNNY_KEY;
        window.IMGBB_KEY = config.IMGBB_KEY;

        console.log("✅ Config loaded from Cloudflare Edge");
        
        initApp();
        
    } catch (error) {
        console.error("❌ Config Load Error:", error);
        // Error ဖြစ်ခဲ့ရင် UI မှာ ပြပေးမယ့် အပိုင်း
        document.body.innerHTML = `
            <div style="text-align:center; padding:50px; font-family:sans-serif;">
                <h2 style="color:red;">⚠️ အက်ပ်စတင်ရန် အဆင်မပြေပါ</h2>
                <p>အင်တာနက်ချိတ်ဆက်မှု စစ်ဆေးပြီး Refresh ပြန်လုပ်ပေးပါ။</p>
            </div>`;
    }
}
...
// အောက်က variable တွေကို သီးသန့် ထားပေးပါ
let selectedFiles = [];
// window.MAIN_POST_TABLE က loadConfig ထဲမှာ သတ်မှတ်ထားဖို့ လိုပါတယ်
const TARGET_TABLE = window.MAIN_POST_TABLE || 'posts';

(function() {
async function getDeviceId() {
    if (typeof FingerprintJS === 'undefined') {
        console.warn("⚠️ FingerprintJS not loaded, retrying...");
        await new Promise(resolve => setTimeout(resolve, 500));
        return getDeviceId(); // Logic ကို ထပ်ခါတလဲလဲ စစ်မယ်
    }
...
async function handleDeviceAutoLogin() {
    try {
        ...
        // (က) လက်ရှိ Session ရှိမရှိ အရင်စစ်မယ်
        ...
        // (ခ) Device ID (Fingerprint) ရယူမယ်
        ...
        // (ဂ) Supabase Anonymous Login ဝင်မယ်
        const { data: authData, error: loginError } = await supabase.auth.signInAnonymously({
        ...
        // (င) Post များကို Load လုပ်ခြင်း သို့မဟုတ် UI Update လုပ်ခြင်း
...
async function getMyDeviceId() {
    try {
        // ၁။ FingerprintJS library ရှိမရှိ အရင်စစ်မယ်
        if (typeof FingerprintJS === 'undefined') return "unknown_id";

        // ၂။ window.fpAgent မရှိသေးရင် Load လုပ်မယ်
        if (!window.fpAgent) {
            window.fpAgent = await FingerprintJS.load();
        }
...
async function loadMorePosts() {
    ...
    // ၁။ Global flags များကို window. သုံးပြီး စစ်ဆေးခြင်း (Safety Guard)
    ...
    try {
        ...
        // ၂။ User Session နှင့် Admin အခြေအနေကို စစ်ဆေးခြင်း
        const { data: { session } } = await supabase.auth.getSession();
        ...
        // ၁၀။ နောက်တစ်ကြိမ် load လုပ်ခွင့်ပေးရန် Flag ကို false ပြန်ထားမယ်
        window.isFetching = false;
    }
}
...
window.openPhotoViewerFromId = function(index, postId) {
    ...
    if (post) {
        ...
        if (typeof openFullViewer === "function") {
            openFullViewer(window.currentIndex, window.photoList);
        } else {
            // UI function မရှိသေးရင် Debugging အတွက် console မှာပြမယ်
...
async function handleReact(postId, type, event, targetTable = 'posts') {
    ...
    // လက်ရှိ React လုပ်ထားသလားဆိုတာကို postData သို့မဟုတ် UI color ကနေ စစ်မယ်
    ...
    // Local Data (window.allPosts) ကိုပါ Update လုပ်မယ်
    if (postData) {
    ...
    // ၄။ Smart Queue Logic (ခြေဖျက်ခြင်း logic)
    if (!window.reactionQueue) window.reactionQueue = [];

    // လက်ရှိ Post + Type အတွက် Queue ထဲမှာ Action တစ်ခုခု ရှိနေသလား ရှာမယ်
    ...
    // ၆။ Background Sync (Online ဖြစ်ရင် ချက်ချင်းပို့မယ်)
    if (navigator.onLine) {
...
function renderComments(postId, comments, isAdmin, currentUid) {
    // ၁။ Data မရှိရင် "မှတ်ချက်မရှိသေးပါ" လို့ ပြမယ်
    if (!comments || comments.length === 0) {
        return `<div style="padding:10px; color:gray; font-size:12px;">မှတ်ချက်မရှိသေးပါ</div>`;
    }
    
    // ၂။ ပထမ ၅ ခုကိုပဲ အရင်ပြမယ် (Initial Display)
    const initialComments = comments.slice(0, 5);
    const extraComments = comments.slice(5); // ကျန်တဲ့ comment များ

    let html = initialComments.map(c => 
        renderCommentHTML(c, isAdmin, currentUid, postId)
    ).join('');
    
    // ၃။ Comment ၅ ခုထက်ကျော်ရင် See More ခလုတ် ထည့်မယ်
    if (comments.length > 5) {
        ...
        html += `
            ...
            <div id="more-btn-${postId}" 
                 ...
                 style="color:var(--primary-color, purple); font-size:13px; cursor:pointer; padding:8px 0; font-weight:600;">
...
function showAllComments(postId) {
    ...
    if (extraDiv) {
        ...
        if (moreBtn) moreBtn.style.display = 'none'; // ခလုတ်ကို ဖျောက်လိုက်မယ်
    }
}
...
function renderPostHTML(id, d, uid, isAdmin, targetTable = 'posts') {
    ...
    if (mUrls.length > 0) {
        if (mType === "video" || mUrls[0].toLowerCase().endsWith(".mp4")) {
            ...
            const posterUrl = videoUrl.includes('b-cdn.net') 
            ...
            mediaHTML = `
                <div style="margin-top:10px; background:#000; border-radius:8px; overflow:hidden; position:relative;">
                    <video class="post-video" 
                           ...
                           webkit-playsinline 
                           poster="${posterUrl}" 
                           style="width:100%; display:block; min-height:200px; max-height:450px; cursor:pointer;" 
    ...
    <div class="post-card" id="post-${id}" data-id="${id}" style="background:white; border-radius:12px; padding:15px; margin-bottom:15px; box-shadow:0 2px 8px rgba(0,0,0,0.1); border: ${isPinned ? '2px solid #6A1B9A' : 'none'};">
        ...
        <div style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:10px;">
            <div style="display:flex; flex-direction:column; flex:1; min-width:0;">
               <b style="color:purple; font-size:15px;">${typeof getDisplayNameWithBadge === 'function' ? getDisplayNameWithBadge(d) : 'User'}</b>
...
// Global အနေနဲ့ အစမှာ ကြေညာထားနိုင်ပါတယ် (optional)
window.photoList = [];
window.currentIndex = 0;

window.openPhotoViewer = function(index, photosJson) {
    try {
        // ၁။ Data ကို Decode လုပ်ပြီး JSON ပြောင်းမယ်
        let rawData = decodeURIComponent(photosJson);
        let parsedData = JSON.parse(rawData);
        
        // ၂။ Array ဖြစ်မဖြစ် စစ်ဆေးပြီး Global variable မှာ သိမ်းမယ်
        ...
        if (viewer && img) {
            ...
            // ၃။ Loading State (ပုံအသစ်မပေါ်ခင် ဝါးထားမယ်)
            img.style.opacity = "0.5"; 
            img.src = window.photoList[window.currentIndex];

            // ၄။ ပုံအမှန်တကယ် Load ဖြစ်သွားမှ အလင်းပြန်တင်မယ်
            img.onload = () => {
                img.style.opacity = "1";
                img.style.transition = "opacity 0.3s ease";
            };

            // ၅။ Error Handling (ပုံ Link ပျက်နေရင်)
            img.onerror = () => {
                // သင့် Logo link အဟောင်း သို့မဟုတ် Placeholder တစ်ခုခု သုံးပါ
            ...
            // ၆။ အောက်ခြေက ဂဏန်း (1/3) စတာတွေကို update လုပ်မယ်
...
async function syncAllData() {
    // ၁။ Connection မရှိရင် သို့မဟုတ် Sync လုပ်နေဆဲဆိုရင် ရပ်မယ်
    if (!navigator.onLine || window.isSyncing) return;

    // ၂။ Data ရှိမရှိ စစ်ဆေးခြင်း (ပိုမိုသန့်ရှင်းသော syntax)
    ...
    try {
        ...
        // --- (က) VIEWS SYNC (Sequential processing to avoid race conditions on counts) ---
        ...
        // --- (ခ) REACTIONS SYNC (Batch processing with Promise.all) ---
        if (window.reactionQueue?.length > 0) {
            ...
            // အောင်မြင်တဲ့ item တွေကိုပဲ queue ထဲက ဖယ်ထုတ်မယ်
            window.reactionQueue = results
                .filter(res => !res.success)
                .map(res => res.item);
        }

        // --- (ဂ) COMMENTS SYNC (With Retry Logic) ---
        ...
        // Local Storage မှာ Data Update လုပ်မယ်
        if (typeof saveAllQueuesToLocal === 'function') {
            saveAllQueuesToLocal();
        }
        window.isSyncing = false;
...
function saveAllQueuesToLocal() {
    // ၁။ window object တွေ ရှိမရှိ အရင်စစ်မယ် (Undefined error မတက်အောင်)
    const dataMap = {
    ...
    try {
        Object.entries(dataMap).forEach(([key, val]) => {
            ...
            if (hasData) {
                ...
                // Data မရှိရင် localStorage ကနေ အပြီးဖျက်မယ်
...
window.changeSlide = function(direction) {
    ...
    // ၂။ Index ကို တွက်ချက်မယ် (window. ထည့်ရမယ်)
    ...
    if (img) {
        ...
        // ၄။ ပုံအသစ်ကို ထည့်မယ် (window.photoList နဲ့ window.currentIndex)
        img.src = window.photoList[window.currentIndex];
...
async function sendUnlockRequest(uid, newDid, name) {
    ...
    try {
        // Developer ဆီ Alert ပို့မယ့် Logic (ဥပမာ- Console မှာ အရင်ကြည့်မယ်)
        console.log(`Alert: User ${name} (${uid}) is requesting unlock for device ${newDid}`);
        
        // ပိုသေချာအောင် Admin Backup Email ကို Console မှာ မှတ်ထားမယ်
        if (window.ADMIN_BACKUP) {
            console.log("Contacting Backup Admin:", window.ADMIN_BACKUP);
        }

        // User ကို Alert ပြမယ် (ဒါက အသေချာဆုံးပဲ)
        alert(`မင်္ဂလာပါ ${name}၊\nDeveloper ထံသို့ Unlock Request ပို့ပြီးပါပြီ။ Admin က အတည်ပြုပြီးပါက ပြန်လည်အသုံးပြုနိုင်ပါပြီ။`);
        
        // အကောင့်ထဲက ထွက်ပြီး Page ကို Reload လုပ်မယ်
        await window.supabase.auth.signOut();
        location.reload();
        
    } catch (e) {
        console.error(e);
        // Error တက်တဲ့အခါမှာတော့ UI မပျက်သွားအောင် Toast သုံးရင် သုံးနိုင်ပါတယ်
        if (typeof showToastMessage === 'function') {
